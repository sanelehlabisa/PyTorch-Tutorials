...
class Generator():
    ...
    def char_tensor(self, string):
        ...
        for c in range(len(string)):
            tensor[c] = all_charecters.index[string[c]]
        return tensor
    ...
    def get_random_batch(self):
        ...
        text_target = torch.zero(self.batch_size, self.chunk_len)
    ...
    def generate(self, initial_str='Ab', prediction_len=100, temperature=0.85):
        ...
        for p in range(len(prediction_len) - 1):
            ...
            predicted_char = all_charecters[top_char]
    ...
    def train(self):
        self.rnn = RNN(n_charecters, self.hidden_size, self.num_layers, n_charecters).to(device)

        optimizer = torch.optim.Adam(self.rnn.parameters(), lr=self.lr)
        criterion = nn.CrossEntropyLoss()
        writer = SummaryWriter(f"runs/name0")
